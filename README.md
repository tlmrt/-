...
- 🪟 桌面小组件：把日期格子拖到桌面变成独立小窗（当天任务 + 液体倒计时），**支持自定义配色**（8 套预设 + 背景 / 文字 / 强调三色取色器，每个小组件独立保存）；另有 **MAA 监视小组件**（状态 / 时长 / 今日计划 + 一键启停）
- ⌘ **命令面板**：`Ctrl+K` 搜任务、跳日期（输入「明天」「10-01」即可）、新建任务、执行命令；`Ctrl+N` 新建、`Ctrl+,` 设置、`T` 回今天、`←/→` 换月
- ✍️ **一句话建任务**：`明天下午3点 开会 提前10分钟` 回车即自动填好日期 / 时间 / 重复 / 提醒
- ⏰ **提醒卡稍后提醒**：点通知可「稍后 10 分钟 / 1 小时」
- 🗄 **数据自动备份**：每天首次启动自动备份，保留 7 份，设置里一键恢复（恢复前会自动再备份一次当前数据）
- 🗂 设置面板按「外观 / 日历 / 提醒与启动 / 联动与 MAA / 更新与插件 / 数据备份」分类，长段说明收进选项旁的 ⓘ 圆圈提示
- 🧩 **插件口**：见下文
...
```bash
npm test   # 549 项单测：提醒引擎 / 节日农历 / 节假日 / 更新 / 开机自启 / 备份 / 稍后提醒 / 一句话解析 / MAA / 本地接口
```
